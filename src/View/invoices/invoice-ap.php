                                            ...
                                            <div class="col-md-8 form-group">
                                                <div id="po-container">
                                                    <div class="row po-item">
                                                        <div class="col-md-10">
                                                            <select name="order_ap_id[0]" class="form-control order-select" id="order_ap_id">
                                                            <option value="" disabled selected hidden>Select</option>
                                                                <?php foreach($vendor as $vnd): ?>
                                                                    <option value="<?= $vnd->order_ap_id ?>"><?= $vnd->no_po ?></option>
                                                                <?php endforeach; ?>
                                                            </select>
                                                        </div>
                                                        <div class="col-md-2">
...
<script>
            ...
            columns:[
                ...
                },  
                {
                    ...
                    render: function(data,type,row){
                        return '<a href="<?= base_url().'/detailtransaction-ap/'?>'+data+'">'+data+'</a>'
                    }
                },
                {
                    data: 'tgl_invoice',
    ...
    function crudInvoices(){
        ...
        $('#invoicepdf').on('click', function(e){
            ...
            selectedData.each(function(data) {
                const uuid = data.uuid;
                window.open('<?= base_url()?>/template-invoice-ap/'+uuid,'_blank');
            })
        })
    }
    function addElement() {
        ...
        var newElement = `
            <div class="row po-item">
                <div class="col-md-10">
                    <select name="order_ap_id[]" class="form-control order-select">
                        <option value="" disabled selected hidden>Select</option>
                        <?php foreach($vendor as $vnd): ?>
                            <option value="<?= $vnd->order_ap_id ?>"><?= $vnd->no_po ?></option>
                        <?php endforeach; ?>
    ...
    function getPricePO(){
        $('#po-container').on('change', '.order-select', function () {
            ...
            if (orderId) {
                $.ajax({
                    type: 'POST',
                    url: '<?= base_url()?>/getpricepo-ap',
                    data: { order_ap_id: orderId },
                    dataType: 'json',
    ...
    function calculateTotal() {
        ...
        $('#subtotal').on('change', function(){
            ...
            var rpjumlah = jumlah.toLocaleString('id-ID');
            $('#total_pembayaran').val(jumlah);
            $('#rptotal_pembayaran').val(rpjumlah);
        })
    }
